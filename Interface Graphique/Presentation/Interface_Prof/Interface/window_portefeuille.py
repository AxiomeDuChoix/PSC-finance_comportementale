...
class MyWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self,Parametres,tabgammas, parent=None):
        ...
        df = pd.read_csv('Interface/frontiere'+str(self.Parametres.numeroExperience)+'.csv', delimiter = ';')
...
#            s=np.sqrt(np.vdot(xtg,np.dot(self.Parametres.sigma,xtg)))
        ...
        if(self.Parametres.riskless_autorise):
            point=self.ui.graphicsView_frontiere.plot([0],[self.Parametres.Rf],symbol='star',symbolBrush=(255,255,255),symbolSize=16,pen=None)
            l.addItem(point,"Actif sans risque")
            yMin=min(self.Parametres.Rf-0.01,yMin)
        self.ui.graphicsView_frontiere.setLimits(xMin=-0.1,xMax=xMax,yMin=yMin,yMax=yMax)
