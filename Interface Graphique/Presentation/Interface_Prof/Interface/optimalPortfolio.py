...
def calculGamma(Std,E,gamma):
    maxi=-1e15
    rang=0
    for i in range(len(Std)):
        if E[i]-gamma*(Std[i]**2)>maxi:
            maxi=E[i]-gamma*(Std[i]**2)
            rang=i
    return Std[rang],E[rang]
...
def bornesAffichageAvecVaD(Std, E):
    ...
    for i in range(len(Std)):
        ...
        if(val < xMin):
            xMin = val
            yMin = E[i]
    return (xMin,xMax,yMin-(yMax-yMin)/2,yMax)
...
plotWidget=pg.plot(X,Y)
plotWidget.setLimits(xMin=xMin,xMax=xMax,yMin=yMin,yMax=yMax)
